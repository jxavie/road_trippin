...
function stateMap(dataset, state) {
    ...
    d3.json("../static/data/state_polygons.json").then((response) => {
        
        console.log(response);
        console.log(state);

        var polygon = response[state];

        // var reversedCoords = coordsToLatLngs(coordinates, coordinates.length);

        // var polygon = L.geoJSON.coordsToLatLng((response[state]));
        // console.log(polygon);
        ...
        var polygonLayer = L.polygon(polygon, polygonStyle);
        ...
        var statemap = L.map("state_map",{
            ...
            // zoom: mapZoom,
            layers: [darkmap, polygonLayer, tunnelLayer],
        ...
        statemap.fitBounds(polygonLayer.getBounds());
    });

...
function overlay_dollars_chart(x,y1,y2,y3,x1,y4,y5) {
  var trace1 = {
    x: x,
    y: y1,
    // text: labels,
...
// define function for state vs. federal spending plot
function overlay_gdp_chart(x,y1,y2,y3) {
  var trace1 = {
    x: x,
    y: y1,
    // text: labels,
...
// define function for global spending plot
function overlay_global_chart(x , y, year, colors) {
  var trace = {
    x: x,
    y: y,
    type: "bar",
  ...
  Plotly.newPlot("overlay_global_spending", data, layout);
};
...
d3.json("/api/spending").then((dataset) => {
  ...
  if(selected_view=="Dollars") {
    overlay_dollars_chart(exp_year, exp_gross, exp_fed, exp_state, year_estimates, cost_maintain, cost_improve);
  }
  else {
    overlay_gdp_chart(exp_year, gdp_gross, gdp_fed, gdp_state);
  };
  ...
  overlay_global_chart(total_perCapita, total_country, selected_year, colors_total);
...
function globalChangedYear(selection) {
  ...
  d3.json("/api/spending").then((dataset) => {
    ...
    if(selectedType == "Road infrastructure investment") {
      overlay_global_chart(inv_perCapita, inv_country, selection, colors_inv);
    }
    else if (selectedType == "Road infrastructure maintenance") {
      overlay_global_chart(om_perCapita, om_country, selection, colors_om);
    }
    else {
      overlay_global_chart(total_perCapita, total_country, selection, colors_total);
    };
...
function globalChangedType(selection) {
  ...
  d3.json("/api/spending").then((dataset) => {
    ...
    if(selection == "Road infrastructure investment") {
      overlay_global_chart(inv_perCapita, inv_country, selectedYear, colors_inv);
    }
    else if (selection == "Road infrastructure maintenance") {
      overlay_global_chart(om_perCapita, om_country, selectedYear, colors_om);
    }
    else {
      overlay_global_chart(total_perCapita, total_country, selectedYear, colors_total);
    };
